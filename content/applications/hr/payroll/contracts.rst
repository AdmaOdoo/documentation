...
.. note::
   Contract templates are created through the **Payroll** or **Employees** apps configuration menu,
   and stored in the **Documents** app.
...
.. note::
   If no contract templates exist, or if a new contract template is needed, :ref:`create a new
   template <payroll/contract-template>`.

.. important::
   Depending on the installed :doc:`payroll localization <payroll_localizations>`, different
   sections and fields may appear in the *Payroll* tab.
...
- :guilabel:`Wage Type`: Select either :guilabel:`Fixed Wage` or :guilabel:`Hourly Wage` from the
  ...
  Wage` for employees who are paid based on their logged worked hours.
...
- :guilabel:`Pay Category`: Select one of the default pay categories using the drop-down menu. The
  ...
  .. note::
     :guilabel:`Pay category` was referred to as :guilabel:`Salary Structure Types` in previous
     versions of Odoo.
...
- :guilabel:`Working Hours`: Select one of the available :guilabel:`Working Hours` the employee is
  expected to work, using the drop-down menu. The selected working hours determines how :doc:`work
  ...
  every week, with no restrictions. The default options are :guilabel:`Standard 40 hours/week`, and
  :guilabel:`Appointment Resource Default Calendar`

  .. note::
     :guilabel:`Working Hours` was previously referred to as :guilabel:`Working Schedules` in
...
- :guilabel:`Signatory`: Using the drop-down menu, select the person that must sign for the
